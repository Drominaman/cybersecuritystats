...
export default async function OgImage({ params }: { params: Promise<{ industry: string; threat: string }> }) {
  const { industry, threat } = await params
  const label = industry.replace(/-/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase())
  const threatLabel = threat.replace(/-/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase())
